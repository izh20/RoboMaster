...
int16_t flag_left = 1, flag_right = 1;
...
void pc_kb_hook(void)
{
  ...
  if (rc.kb.bit.A)
	...
	//Webb begin
	if (rc.kb.bit.E && flag_left==1)
	{
    set_pwm_param(PWM_IO3, 800);
    //write_led_io(LASER_IO, LED_ON);
		flag_left = 0;
  }
  if (rc.kb.bit.R && flag_left==0)
	{
    set_pwm_param(PWM_IO3, 1050);
    //write_led_io(LASER_IO, LED_OFF);
		flag_left = 1;
  }
	
	if (rc.kb.bit.V && flag_right==0)
	{
    set_pwm_param(PWM_IO4, 1000);
    //write_led_io(LASER_IO, LED_ON);
		flag_right = 1;
  }
	if (rc.kb.bit.B && flag_right==1)
	{
    set_pwm_param(PWM_IO4, 500);
    //write_led_io(LASER_IO, LED_OFF);
		flag_right = 0;
  }
	//Webb end	
