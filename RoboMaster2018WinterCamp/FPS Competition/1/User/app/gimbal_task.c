...
void read_gimbal_offset(int32_t *pit_offset, int32_t *yaw_offset)
{
  ...
  //start_pwm_output(PWM_IO3);		//Webb_Bing
  //start_pwm_output(PWM_IO4);		//Webb_Bing
...
void gimbal_init_handle(void)
{
  ...
  switch (gimbal_back_step)
  {
    ...
    case BACK_IS_OK:
    {
      ...
      pid_pit_speed.max_output = 5000;			//Webb_Bing 
    }break;
  }
}
...
void gimbal_init_param(void)
{
  ...
  osDelay(3000);
	...
	//Webb_Bing begin
  /* 云台pitch轴电机PID参数初始化 */
  pid_init(&pid_pit, 5000, 0,
                  35, 0, 0); //
  pid_init(&pid_pit_speed, 7000, 2000,
                  25, 0.1, 0);
...
void block_bullet_handle(void)
{
  ...
  if (stall_count >= 200) //卡弹时间3s								//Webb_Bing
  {
...
uint16_t  fric_wheel_speed = 1180;
/* 上次的遥控数据数据 */
...
//遥控器连发
//Webb_Bing
#define RC_CONTIN_TRIG   ((rc.sw1 == RC_DN) && (HAL_GetTick() - continue_shoot_time >= 10))   //((rc.wheel == 660) && (HAL_GetTick() - continue_shoot_time >= 1000))
//遥控器退出连发模式
