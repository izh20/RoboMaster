...
moto_measure_t w_2006_moto_ID_5_plus[4];
/* 底盘电机 */
moto_measure_t moto_chassis[4];

moto_measure_t moto_3508_ID_1;
...
void can1_recv_callback(uint32_t recv_id, uint8_t data[])
{
  switch (recv_id)
  {
    ...
    {
      encoder_data_handle(&w_2006_moto_ID_5_plus[0], data);
      err_detector_hook(GIMBAL_YAW_OFFLINE);
    }
    break;
    case CAN_PIT_MOTOR_ID:
    {
      encoder_data_handle(&w_2006_moto_ID_5_plus[1], data);
      err_detector_hook(GIMBAL_PIT_OFFLINE);
    }
    break;
    case CAN_TRIGGER_MOTOR_ID:
    {
      w_2006_moto_ID_5_plus[2].msg_cnt++;
      w_2006_moto_ID_5_plus[2].msg_cnt <= 10 ? get_moto_offset(&w_2006_moto_ID_5_plus[2], data) : encoder_data_handle(&w_2006_moto_ID_5_plus[2], data);
      err_detector_hook(TRIGGER_MOTO_OFFLINE);
    }
    break;
    case CAN_test_moto_ID:
    {
      w_2006_moto_ID_5_plus[3].msg_cnt++ <= 50 ? get_moto_offset(&w_2006_moto_ID_5_plus[3], data) : \
      encoder_data_handle(&w_2006_moto_ID_5_plus[3], data);
...
void can2_recv_callback(uint32_t recv_id, uint8_t data[])
{
  switch (recv_id)
  {
    ...
    case CAN_3508_M1_ID:
    {
      moto_3508_ID_1.msg_cnt++ <= 50 ? get_moto_offset(&moto_3508_ID_1, data) : \
      encoder_data_handle(&moto_3508_ID_1, data);
    }
    break;
    default:
    {
