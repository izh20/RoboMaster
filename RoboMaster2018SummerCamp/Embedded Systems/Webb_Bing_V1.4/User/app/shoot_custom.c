...
void shoot_custom_control(void)
{
  if (fric_wheel_run)
  {
    ...
    if (shoot_cmd)
    {
      trigger_moto_position_ref = w_2006_moto_ID_5_plus[2].total_ecd + DEGREE_60_TO_ENCODER;
      shoot_cmd = 0;
    }
    /* 闭环计算拨弹电机期望转速 */
    trigger_moto_speed_ref = pid_calc(&pid_trigger, w_2006_moto_ID_5_plus[2].total_ecd, trigger_moto_position_ref);
   ...
   // block_bullet_handle();                                 //卡弹处理
    ...
    /* 闭环计算拨弹电机电流 */
    trigger_moto_current = pid_calc(&pid_trigger_speed, w_2006_moto_ID_5_plus[2].speed_rpm, trigger_moto_speed_ref);
  }
  else
  {
